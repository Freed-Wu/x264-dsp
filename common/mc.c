...
static inline void pixel_avg(pixel *restrict dst, intptr_t i_dst_stride,
	...
	switch (i_width) {
	case 20:
		pixel_avg_w20_ti(dst, i_dst_stride, src1, i_src1_stride, src2, i_src2_stride, i_width, i_height);
		break;
	case 16:
		pixel_avg_w16_ti(dst, i_dst_stride, src1, i_src1_stride, src2, i_src2_stride, i_width, i_height);
		break;
	case 12:
		pixel_avg_w12_ti(dst, i_dst_stride, src1, i_src1_stride, src2, i_src2_stride, i_width, i_height);
		break;
	case 8: {
	...
	for (y = 0; y < i_height; y++) {
		for (x = 0; x < i_width; x++) {
			dstu[x] = (cA * src[2 * x] + cB * src[2 * x + 2] +
				   cC * srcp[2 * x] + cD * srcp[2 * x + 2] + 32) >>
				  6;
			dstv[x] = (cA * src[2 * x + 1] + cB * src[2 * x + 3] +
				   cC * srcp[2 * x + 1] + cD * srcp[2 * x + 3] + 32) >>
				  6;
		}
		dstu += i_dst_stride;
		dstv += i_dst_stride;
