...
enum nal_unit_type_e {
	...
	NAL_AUD = 9,	   /* access unit delimiter, only for special scenario */
...
typedef struct x264_param_t {
	...
	struct
	{
		 ...
		 * if mv_range is not set by user, use value defined by H.264 level. *
	...
	struct
	{
		 ...
		 * set the last one.                                                                      *
		 ******************************************************************************************/
		int i_vbv_max_bitrate;	 /* set by --vbv-maxrate. default: 0 */
		int i_vbv_buffer_size;	 /* set by --vbv-bufsize. default: 0 */
		float f_vbv_buffer_init; /* <=1: fraction of buffer_size. >1: kbit. set by --vbv-init. default: 0.9 */
	...
	int b_vfr_input;	 /* VFR input.  If 1, use timebase and timestamps for ratecontrol purposes.
				  * If 0, use fps only. */
	int b_pulldown;		 /* use explicit set timebase for CFR */
