...
#ifdef _DEBUG
	if (res) {
		int num_mb = (h->mb.i_mb_width > 2 && h->mb.i_mb_height > 2) ? (h->mb.i_mb_width - 2) * (h->mb.i_mb_height - 2) : (h->mb.i_mb_width * h->mb.i_mb_height);
...
void x264_slicetype_decide(x264_t *h) {
	 ...
	 ************************************************************/
	if ((h->param.i_bframe && h->param.i_bframe_adaptive) || h->param.i_scenecut_threshold || h->param.rc.b_mb_tree || (h->param.rc.i_vbv_buffer_size && h->param.rc.i_lookahead))
