...
static int x264_validate_parameters(x264_t *h, int b_open) {
	...
	/* h.264 profile and level indication */
	{
		...
		/* if mv_range is not set by user, use value defined by current level. */
...
static void mbcmp_init(x264_t *h) {
	...
	h->pixf.intra_mbcmp_x9_4x4 = h->param.b_cpu_independent ? NULL : satd ? h->pixf.intra_satd_x9_4x4
...
x264_t *x264_encoder_open(x264_param_t *param) {
	...
	profile = h->sps->i_profile_idc == PROFILE_BASELINE ? "Constrained Baseline" : h->sps->i_profile_idc == PROFILE_MAIN ? "Main"
															     : "High";
...
static inline void x264_reference_hierarchy_reset(x264_t *h) {
	...
	for (i = 0; h->frames.current[i] && IS_DISPOSABLE(h->frames.current[i]->i_type); i++)
		b_hasdelayframe |= h->frames.current[i]->i_coded != h->frames.current[i]->i_frame + h->sps->vui.i_num_reorder_frames;
	...
	for (ref = 0; h->frames.reference[ref]; ref++) {
		if ((h->param.i_bframe_pyramid == X264_B_PYRAMID_STRICT && h->frames.reference[ref]->i_type == X264_TYPE_BREF) || (h->frames.reference[ref]->i_poc < h->frames.i_poc_last_open_gop && h->sh.i_type != SLICE_TYPE_B)) {
...
static int x264_slice_write(x264_t *h) {
	...
	reencode:
		...
		if (h->param.i_log_level >= X264_LOG_INFO) {
			if (h->mb.i_cbp_luma | h->mb.i_cbp_chroma) {
				int cbpsum = (h->mb.i_cbp_luma & 1) + ((h->mb.i_cbp_luma >> 1) & 1) + ((h->mb.i_cbp_luma >> 2) & 1) + (h->mb.i_cbp_luma >> 3);
			...
			h->stat.frame.i_mb_field[b_intra ? 0 : b_skip ? 2
								      : 1] += MB_INTERLACED;
		}
#endif
	...
	if (h->sh.i_last_mb == (h->i_threadslice_end * h->mb.i_mb_width - 1)) {
		h->stat.frame.i_misc_bits = bs_pos(&h->out.bs) + (h->out.i_nal * NALU_OVERHEAD * 8) - h->stat.frame.i_tex_bits - h->stat.frame.i_mv_bits;
